<template>
  <div class="relative w-full max-w-md mx-auto" ref="autocompleteContainer">
    ...
    <div :style="dropdownStyle" v-if="isShow" class="max-h-60 overflow-auto z-10 mt-1">
      <!-- Loading Spinner -->
      <div
      ...
      <ul v-else-if="filteredOptions?.length" class="bg-white border border-gray-300 rounded-md">
        <li
...
const dropdownStyle: { [key: string]: any } = computed(() => {
  ...
  return {
    ...
    left: shouldFix ? `${autocompleteContainer.value?.getBoundingClientRect().left}px` : 'auto',
